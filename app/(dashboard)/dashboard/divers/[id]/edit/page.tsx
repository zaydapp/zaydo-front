...
export default function EditDiversPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('divers.updated'));
      router.push('/dashboard/divers');
    },
    onError: () => {
      toast.error(t('divers.updateError'));
    },
  });
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('divers.edit')}</h1>
          <p className="text-muted-foreground mt-1">{expense.name}</p>
        </div>
      </div>
